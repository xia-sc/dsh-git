...
window.__ModuleLoader__.load({
	...
	var zh = {
		...
		"action.stage": "暂存全部",
		"action.push": "推送",
		"action.generateMessage": "AI 生成",
		"action.generateMode.all": "全部",
		"action.generateMode.staged": "已暂存",
		"action.generateMode.unstaged": "未暂存",
	...
	var en = {
		...
		"action.push": "Push",
		"action.generateMessage": "AI Generate",
		"action.generateMode.all": "All",
		"action.generateMode.staged": "Staged",
		"action.generateMode.unstaged": "Unstaged",
		...
		"busy.generateMessage": "Generating…",
		...
		"generate.noStaged": "No staged changes",
		...
		function createGitStore(ctx) {
			var verbs = {
				...
				generateMessage: function (cwd, mode, provider, model) { return rpc(ctx, "generateMessage", { cwd: cwd, mode: mode || "all", provider: provider, model: model }); }
			};
		...
		function GitFloatingPanel(props) {
			...
			var currentSummary = currentId !== undefined && sessions && sessions.byId ? sessions.byId[currentId] : undefined;
			var cwd = currentSummary ? currentSummary.cwd : undefined;

			// Extract current session's model selection (provider + model)
			var sessionModelSelection = currentSummary && currentSummary.projectionValues && currentSummary.projectionValues.modelSelection;
			var sessionProvider = null;
			var sessionModel = null;
			if (sessionModelSelection) {
				var effective = sessionModelSelection.next || sessionModelSelection.lastUsed;
				if (effective) {
					sessionProvider = effective.provider;
					sessionModel = effective.model;
				}
			}
			...
			var generatingRef = React.useState(false);
			var generating = generatingRef[0];
			var setGenerating = generatingRef[1];
			var generateModeRef = React.useState("all");
			...
			bodyChildren.push(h("div", { key: "commit", style: S.row }, [
				h("input", {
					...
					onKeyDown: function (event) {
						if (event.key === "Enter" && message.trim() !== "" && busy === null && !generating) {
							doCommit();
						}
					}
				}),
				h("select", {
					key: "mode",
					style: Object.assign({}, S.select, { width: "auto", minWidth: 60, flex: "none" }),
					value: generateMode,
					disabled: busy !== null || generating,
					onChange: function (event) { setGenerateMode(event.target.value); },
					"aria-label": "Generate mode"
				}, [
					h("option", { key: "all", value: "all", style: OPTION_STYLE }, t("action.generateMode.all")),
					h("option", { key: "staged", value: "staged", style: OPTION_STYLE }, t("action.generateMode.staged")),
					h("option", { key: "unstaged", value: "unstaged", style: OPTION_STYLE }, t("action.generateMode.unstaged"))
				]),
				h("button", {
					key: "ai-generate",
					type: "button",
					style: Object.assign({}, S.button, (busy !== null || generating) ? S.buttonDisabled : null),
					disabled: busy !== null || generating,
					title: t("action.generateMessage"),
					"aria-label": t("action.generateMessage"),
					onClick: doGenerateMessage
				}, generating ? t("busy.generateMessage") : "✨ " + t("action.generateMessage")),
				h("button", {
					key: "stage",
					type: "button",
					style: Object.assign({}, S.button, (busy !== null || generating || state.dirty === 0) ? S.buttonDisabled : null),
					disabled: busy !== null || generating || state.dirty === 0,
					title: state.dirty === 0 ? t("changes.none") : t("action.stage"),
					onClick: function () { store.act("stage", function () { return store.verbs.stage(state.cwd); }); }
				}, busy === "stage" ? t("busy.stage") : t("action.stage")),
				h("button", {
					key: "commit",
					type: "button",
					style: Object.assign({}, S.buttonPrimary, (busy !== null || generating || message.trim() === "") ? S.buttonDisabled : null),
					disabled: busy !== null || generating || message.trim() === "",
			...
			function doCommit() {
				...
				if (text === "" || busy !== null || generating) return;
				setMessage("");
				store.act("commit", function () { return store.verbs.commit(state.cwd, text); });
			}

			function doGenerateMessage() {
				if (busy !== null || generating) return;
				setGenerating(true);
				store.verbs.generateMessage(state.cwd, generateMode, sessionProvider, sessionModel).then(function (res) {
					if (res && res.ok === true && res.value && res.value.message) {
						setMessage(res.value.message);
					} else {
						var errMsg = res && res.error && res.error.message ? res.error.message : t("generate.error");
						store.act("generateMessage", function () { return Promise.resolve(res); });
					}
					setGenerating(false);
				}).catch(function () {
					setGenerating(false);
