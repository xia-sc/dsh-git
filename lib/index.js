...
const inject = ["connection", "llm", "credentials"];
 ...
 */
/**
 * `stage` endpoint: stage all working-tree changes so a subsequent commit
 * succeeds. This is the UI's "暂存全部" path — the panel otherwise only
 * commits what is already staged.
 * @param rawCwd - session workspace directory.
 * @param signal - transport cancellation.
 */
async function gitStage(rawCwd, signal) {
	...
	let out;
	try {
		out = await runGit(rawCwd, ["add", "-A"], signal);
	} catch (error) {
		if (isNotARepo(error)) return fail("not-a-repo", "not a git repository", {});
...
/**
 * Read staged diff for commit message generation.
 * @param rawCwd - session workspace directory.
 * @param signal - transport cancellation.
 */
async function gitDiffCached(rawCwd, signal) {
	if (!validCwd(rawCwd)) return fail("invalid-cwd", "a valid absolute working directory is required");
	let statOut;
	let diffOut;
	try {
		[statOut, diffOut] = await Promise.all([
			runGit(rawCwd, ["diff", "--cached", "--stat"], signal),
			runGit(rawCwd, ["diff", "--cached"], signal, 10000)
		]);
	} catch (error) {
		if (isNotARepo(error)) return fail("not-a-repo", "not a git repository", {});
		return gitError(error);
	}
	const stat = statOut.stdout.trim();
	const diff = diffOut.stdout.trim();
	return ok({ stat, diff });
}

/**
 * Read unstaged diff (working tree changes) for commit message generation.
 * @param rawCwd - session workspace directory.
 * @param signal - transport cancellation.
 */
async function gitDiffUnstaged(rawCwd, signal) {
	if (!validCwd(rawCwd)) return fail("invalid-cwd", "a valid absolute working directory is required");
	let statOut;
	let diffOut;
	try {
		[statOut, diffOut] = await Promise.all([
			runGit(rawCwd, ["diff", "--stat"], signal),
			runGit(rawCwd, ["diff"], signal, 10000)
		]);
	} catch (error) {
		if (isNotARepo(error)) return fail("not-a-repo", "not a git repository", {});
		return gitError(error);
...
/**
 * Read all changes (staged + unstaged) for commit message generation.
 * @param rawCwd - session workspace directory.
 * @param signal - transport cancellation.
 */
async function gitDiffAll(rawCwd, signal) {
	if (!validCwd(rawCwd)) return fail("invalid-cwd", "a valid absolute working directory is required");
	let stagedStatOut;
	let stagedDiffOut;
	let unstagedStatOut;
	let unstagedDiffOut;
	try {
		[stagedStatOut, stagedDiffOut, unstagedStatOut, unstagedDiffOut] = await Promise.all([
			runGit(rawCwd, ["diff", "--cached", "--stat"], signal),
			runGit(rawCwd, ["diff", "--cached"], signal, 10000),
			runGit(rawCwd, ["diff", "--stat"], signal),
			runGit(rawCwd, ["diff"], signal, 10000)
	...
	const stagedStat = stagedStatOut.stdout.trim();
	const stagedDiff = stagedDiffOut.stdout.trim();
	const unstagedStat = unstagedStatOut.stdout.trim();
	const unstagedDiff = unstagedDiffOut.stdout.trim();
	return ok({
		staged: { stat: stagedStat, diff: stagedDiff },
		unstaged: { stat: unstagedStat, diff: unstagedDiff },
		combined: [stagedDiff, unstagedDiff].filter(Boolean).join("\n\n"),
		combinedStat: [stagedStat, unstagedStat].filter(Boolean).join("\n")
	});
}

/**
 * Read recent commits for commit message context.
 * @param rawCwd - session workspace directory.
 * @param count - number of recent commits.
 * @param signal - transport cancellation.
 */
async function gitRecentCommits(rawCwd, count, signal) {
	if (!validCwd(rawCwd)) return fail("invalid-cwd", "a valid absolute working directory is required");
	const n = Number.isInteger(count) ? Math.min(5, Math.max(1, count)) : 3;
	let out;
	try {
		out = await runGit(rawCwd, ["log", `--max-count=${n}`, "--format=%s"], signal);
	} catch (error) {
		if (isNotARepo(error)) return ok({ repo: false, commits: [] });
		return gitError(error);
	}
...
/**
 * Generate a commit message using the LLM service.
 * @param ctx - plugin context with the `llm` and `credentials` services.
 * @param diff - the staged diff content.
 * @param stat - the diff stat summary.
 * @param signal - transport cancellation.
 */
async function generateCommitMessageWithLlm(ctx, diff, stat, signal, sessionProvider, sessionModel) {
	// Truncate diff if too long (keep first 3000 chars for context)
	const truncatedDiff = diff.length > 3000 ? diff.slice(0, 3000) + "\n... (truncated)" : diff;
	
	const systemPrompt = [
		"You are a git commit message generator.",
		"Analyze the provided git diff and generate a concise, semantic commit message.",
		"Rules:",
		"- Use the language of the codebase (if code comments are in Chinese, use Chinese)",
		"- Keep it under 72 characters for the subject line",
		"- Use imperative mood (e.g., 'Add', 'Fix', 'Update', 'Remove')",
		"- Focus on WHAT changed, not HOW",
		"- If multiple files changed, describe the overall purpose",
	...
	// Use session provider if available, otherwise first available
	const providers = ctx.llm.listProviders();
	if (providers.length === 0) {
		throw new Error("No LLM provider available. Please configure a model in settings.");
	}
	
	// Prefer session's provider, fallback to first available
	const selectedProvider = sessionProvider || providers[0].id;
	
	// Prefer session's model, fallback to settings, fallback to first available model
	let selectedModel = sessionModel;
	if (!selectedModel) {
		try {
			const settings = ctx.settings?.get?.("llm");
	...
	if (!selectedModel) {
		...
		try {
			const models = await ctx.llm.listModels(selectedProvider);
			if (models.length > 0) {
	...
	if (!selectedModel) selectedModel = "deepseek-chat";

	// Debug: log provider and model info
	console.log("[dsh-git] LLM request:", { provider: selectedProvider, model: selectedModel, hasCredentials: typeof ctx.get === "function" });

	// Check credentials service availability
	const credentialsSvc = typeof ctx.get === "function" ? ctx.get("credentials") : undefined;
	console.log("[dsh-git] credentials service available:", credentialsSvc !== undefined);

	// Try to resolve the API key to verify credentials work
	if (credentialsSvc !== undefined && typeof credentialsSvc.resolve === "function") {
		try {
			const resolved = await credentialsSvc.resolve("DEEPSEEK_API_KEY");
			console.log("[dsh-git] API key resolved:", resolved ? { source: resolved.source, hasValue: true } : { source: null, hasValue: false });
		} catch (e) {
			console.error("[dsh-git] Failed to resolve API key:", e.message);
		}
	}

	// Create proper DSH message format
	const messages = [{
		role: "user",
	...
	try {
		for await (const chunk of ctx.llm.stream(options)) {
			...
			if (chunk.type === "finish") {
				if (chunk.reason?.kind === "error") {
					const errMsg = chunk.reason.failure?.message || "LLM generation failed";
					const errCode = chunk.reason.failure?.code || "";
					console.error("[dsh-git] LLM error:", { code: errCode, message: errMsg });
					// Provide more helpful error message for auth failures
					if (errCode === "AUTH" || errMsg.includes("api key") || errMsg.includes("authentication")) {
						throw new Error("API key authentication failed. Please check your API key in Settings > Models.");
					}
		...
		console.error("[dsh-git] LLM exception:", error);
		// Re-throw with better context
		if (error.message.includes("API key") || error.message.includes("authentication")) {
			throw error;
		}
		if (error.message.includes("NO_ADAPTER")) {
			throw new Error("No LLM adapter available. Please configure a model in Settings > Models.");
		}
		throw error;
	}

	const message = result.trim();
	if (!message) throw new Error("LLM returned empty commit message");
	return message;
}

/**
 * Plugin body: mount the `/dsh-git-rpc` channel on the shared transport.
 * @param ctx - plugin context with the `connection` and `llm` services.
 */
function apply(ctx) {
	ctx.connection.rpc.handle("/dsh-git-rpc", async (endpoint, payload, signal) => {
		...
		switch (endpoint) {
			...
			case "diffCached":
				return gitDiffCached(args.cwd, signal);
			case "diffUnstaged":
				return gitDiffUnstaged(args.cwd, signal);
			case "diffAll":
				return gitDiffAll(args.cwd, signal);
			case "recentCommits":
				return gitRecentCommits(args.cwd, args.count, signal);
			case "generateMessage": {
				// mode: 'staged' | 'unstaged' | 'all' (default: 'all')
				const mode = args.mode || "all";
				const sessionProvider = args.provider;
				const sessionModel = args.model;
				let diffResult;
				if (mode === "staged") {
					diffResult = await gitDiffCached(args.cwd, signal);
					if (diffResult.ok !== true) return diffResult;
					if (!diffResult.value.diff) return fail("no-changes", "no staged changes to generate commit message");
					try {
						const commitMsg = await generateCommitMessageWithLlm(ctx, diffResult.value.diff, diffResult.value.stat, signal, sessionProvider, sessionModel);
						return ok({ message: commitMsg, mode });
					} catch (error) {
						return fail("generate-failed", error.message || "failed to generate commit message");
					}
				} else if (mode === "unstaged") {
					diffResult = await gitDiffUnstaged(args.cwd, signal);
					if (diffResult.ok !== true) return diffResult;
					if (!diffResult.value.diff) return fail("no-changes", "no unstaged changes to generate commit message");
					try {
						const commitMsg = await generateCommitMessageWithLlm(ctx, diffResult.value.diff, diffResult.value.stat, signal, sessionProvider, sessionModel);
						return ok({ message: commitMsg, mode });
					} catch (error) {
					...
					// 'all' - combined staged + unstaged
					diffResult = await gitDiffAll(args.cwd, signal);
					if (diffResult.ok !== true) return diffResult;
					if (!diffResult.value.combined) return fail("no-changes", "no changes to generate commit message");
					try {
						const commitMsg = await generateCommitMessageWithLlm(ctx, diffResult.value.combined, diffResult.value.combinedStat, signal, sessionProvider, sessionModel);
						return ok({ message: commitMsg, mode });
					} catch (error) {
						return fail("generate-failed", error.message || "failed to generate commit message");
					}
